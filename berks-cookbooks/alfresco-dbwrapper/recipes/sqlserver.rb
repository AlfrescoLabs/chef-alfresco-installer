...
installer_file_path = win_friendly_path(File.join(Chef::Config[:file_cache_path], "sqlserver.exe"))
config_file_path = win_friendly_path(File.join(Chef::Config[:file_cache_path], 'Configuration.ini'))
...
windows_task "Install #{node['sql_server']['server']['package_name']}" do
  user 'Administrator'
  password 'alfresco'
  command "#{installer_file_path} /q /ConfigurationFile=#{config_file_path}"
  run_level :highest
  frequency :monthly
  action [:create, :run]
  not_if { Registry.key_exists?('HKLM\SOFTWARE\Microsoft\Microsoft SQL Server') }
end

batch 'Waiting for installation to finish ...' do
  code <<-EOH
  dir /S /P \"C:\\Program Files\\Microsoft SQL Server\\MSSQL11.SQLEXPRESS\\MSSQL\\Log\\ERRORLOG\"
  EOH
  action :run
  retries 40
  retry_delay 10
  notifies :delete, "windows_task[Install #{node['sql_server']['server']['package_name']}]", :delayed
  not_if { File.exists?("C:\\Program Files\\Microsoft SQL Server\\MSSQL11.SQLEXPRESS\\MSSQL\\Log\\ERRORLOG") }
end
