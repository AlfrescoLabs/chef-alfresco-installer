...
describe 'Validate mysql installation' do
  ...
  context 'When we check if we can connect to mysql with default alfresco/alfresco user the mysql stdout' do
    it do
      expect(command("mysql -h172.29.101.52 -ualfresco -palfresco alfresco -e \"show processlist;\"")
  ...
  context 'When we check if the alfresco database exists the mysql stdout' do
    it do
      expect(command("mysql -h172.29.101.52 -ualfresco -palfresco alfresco -e \"show databases;\"")
  ...
  context 'When we check if we can create a table the mysql stdout' do
    it do
      expect(command("mysql -h172.29.101.52 -ualfresco -palfresco alfresco -e \"create table test(col1 varchar(10));\"")
  ...
  context 'When we check if we can insert row "magic" in the table the mysql stdout' do
    it do
      expect(command("mysql -h172.29.101.52 -ualfresco -palfresco alfresco -e \"insert into test values ('magic');\"")
      .stdout).not_to include('ERROR')
    end
  end
  ...
  context 'When we check if we can select from table the mysql stdout' do
    it do
      expect(command('mysql -h172.29.101.52 -ualfresco -palfresco alfresco << EOF
      select * from test;
      exit
