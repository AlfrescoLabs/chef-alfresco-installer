...
describe 'When alfresco is up the http status' do
  it { expect(connection.get('/alfresco/faces/jsp/login.jsp').status).to eq 200 }
end
...
describe 'When we have solr4 enabled on the admin console page the body' do
  it { expect(connection2.get('/alfresco/s/enterprise/admin/admin-systemsummary').body).to match(">Solr.*\n.*\n.*Enabled") }
end
