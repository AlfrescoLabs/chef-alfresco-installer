...
when 'solaris2'
  case node['oracle_version']
  when '12'
  
  node.default['url']['oracle1'] = 'ftp://172.29.101.56/databases/solaris.x64_12102_database_1of2.zip'
  node.default['url']['oracle2'] = 'ftp://172.29.101.56/databases/solaris.x64_12102_database_2of2.zip'
...
export CLASSPATH=#{node['oracle']['home']}/jlib:#{node['oracle']['home']}/rdbms/jlib" >> root/.bash_profile
  ...
  node.default['url']['oracle1'] = 'ftp://172.29.101.56/databases/solaris.x64_11gR2_database_1of2.zip'
  node.default['url']['oracle2'] = 'ftp://172.29.101.56/databases/solaris.x64_11gR2_database_2of2.zip'
  ...
  groups = {'dba' => 202, 'backupdba' => 203, 'oper' => 204, 'oinstall' => 201}
  groups.each do |grpname, grpid|
    group grpname do
      gid grpid
    end
  ...
  execute 'Add oracle to db groups' do
    command "usermod -G +#{groups['dba']},#{groups['backupdba']},#{groups['oper']},#{groups['oinstall']} oracle"
  end
  ...
  remote_file '/opt/oracle1.zip' do
    ...
    source node['url']['oracle1']
    :create_if_missing
    not_if { File.exist?("/opt/oracle1.zip") }
  end
  ...
  remote_file '/opt/oracle2.zip' do
    ...
    source node['url']['oracle2']
    :create_if_missing
    not_if { File.exist?("/opt/oracle2.zip") }
  end
  ...
  ipadm set-prop -p smallest_anon_port=9000 udp
  ipadm set-prop -p largest_anon_port=65500 udp
  /usr/gnu/bin/sed -i 's#::1             localhost##g' /etc/hosts
  EOH
    not_if 'cat /etc/project | grep default:3::::project.max-sem-ids | grep priv,256,deny | grep project.max-shm-memory'
  end
  ...
  file "#{node['oracle']['downloaddir']}/db_install.rsp" do
    ...
    group 'oinstall'
    mode '0775'
    content "oracle.install.responseFileVersion=/oracle/install/rspfmt_dbinstall_response_schema_v11_2_0
  oracle.install.option=INSTALL_DB_AND_CONFIG
  ORACLE_HOSTNAME=#{node['fqdn']}
  UNIX_GROUP_NAME=oinstall
  INVENTORY_LOCATION=#{node['oracle']['inventory']}
  ...
  execute 'Run oracle installer' do
    ...
    cwd node['oracle']['downloaddir']
    #command "su oracle -c 'ulimit -n 65536 && ulimit -s 32768 && ./runInstaller -showProgress -silent -waitforcompletion -ignoreSysPrereqs -responseFile #{node['oracle']['downloaddir']}/db_install.rsp'"
    command "su oracle -c 'ulimit -n 65536 && ulimit -s 32768 && ./runInstaller -silent -waitforcompletion -ignoreSysPrereqs -responseFile #{node['oracle']['downloaddir']}/db_install.rsp'"
    #returns 6
    only_if { node['oracle']['installoracle'] }
    not_if { File.exist?("#{node['oracle']['home']}/bin/sqlplus") }
  end
